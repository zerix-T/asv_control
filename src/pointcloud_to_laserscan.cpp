...
#include "asv_control/pointcloud_to_laserscan.hpp"
...
{
    PointcloudToLaserscan::PointcloudToLaserscan(const rclcpp::NodeOptions &options)
    : rclcpp::Node("pointcloud_to_laserscan", options)
    {   
        target_frame_ = this->declare_parameter("target_frame", "");
        tolerance_ = this->declare_parameter("tranform_tolerance", 0.01);
        ...
        if (!target_frame_.empty()) {
            ...
            std::bind(&PointcloudToLaserscan::PointcloudCallback, this, _1));
        } else {  // otherwise setup direct subscription
            sub_.registerCallback(std::bind(&PointcloudToLaserscan::PointcloudCallback, this, _1));
        }

        subscription_listener_thread_ = std::thread(
            std::bind(&PointcloudToLaserscan::SubListenerThreadLoop, this));
    ...
    void PointcloudToLaserscan::SubListenerThreadLoop()
    {
    ...
    void PointcloudToLaserscan::PointcloudCallback(sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud_msg)
    {
...
RCLCPP_COMPONENTS_REGISTER_NODE(asv_control::PointcloudToLaserscan)
